...
class plot_window(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.plot_ui.stoppltbtn.clicked.connect(self.stop_btn)

        date = str(dt.today())
        self.filename = date + '_' + itemcode +'-'+ part
        self.baudrate = 9600
        self.port = ''
        ...
        self.plt_item.setTitle(title='Load vs Displacement curve for {}'.format(self.filename), **styles_title)
    ...
    def get_data(self):
        ...
        self.Ym[:-1] = self.Ym[1:]
        self.Zm[:-1] = self.Zm[1:]
        # print('GETDATA..... SHIFT.....',self.Xm, self.Ym, self.Zm)
        ...
        self.Zm = linspace(0,0,self.windowWidth)
        self.ptr = -self.windowWidth
        # print('GETDATA........LINSPACE.......',self.Xm, self.Ym, self.Zm)
    ...
    def update(self):
        ...
        self.Xm[-1] = (float(self.value[0])+240-140)/25.4              # vector containing the instantaneous self.values
        self.Ym[-1] = float(self.value[1]) * 2.20462
        self.Zm[-1] = (float(self.value[2]) - 1.66) * 2.20462      
        ...
        # csv = threading.Thread(target=self.to_csv, args=(self.value[0], self.value[1], self.value[2]))
        # csv.start()
        # csv.join()
    ...
    def save_btn(self):
        ...
        exporter.export('./plots/{}.png'.format(self.filename))
        msg = QtWidgets.QMessageBox()
        msg.setIcon(QtWidgets.QMessageBox.Information)
